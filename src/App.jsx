...
function Shell({
  ...
  /* preview mode + resizing */
  const [previewMode, setPreviewMode] = useState('desktop')
  const [customWidth,  setCustomWidth]  = useState(null)
  const [isDragging,   setIsDragging]   = useState(false)
  const dragRef = useRef({ startX: 0, startWidth: 0, side: 'right' })
  ...
  const widthTransition = isDragging ? 'none' : 'width 0.3s ease'
  ...
  function startDrag(e, side) {
    e.preventDefault()
    dragRef.current = { startX: e.clientX, startWidth: currentWidth, side }
    setPreviewMode(null)
    setIsDragging(true)
  }
  ...
  useEffect(() => {
    ...
    function onMove(e) {
      const { startX, startWidth, side } = dragRef.current
  ...
  const cardContents = (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <SortableContext items={visibleKeys} strategy={verticalListSortingStrategy}>
        {visibleKeys.map(key => (
          <SortableCanvasItem key={key} id={key} onSelect={setSelectedComponent}>
            {PREVIEWS[key]}
          </SortableCanvasItem>
        ))}
      </SortableContext>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: t.shell, overflow: 'hidden', transition: allTransition }}>
      ...
      <nav style={{
        ...
        {/* Component list */}
        <ul style={{ flex: 1, margin: 0, padding: leftOpen ? '0 8px' : '8px 6px', listStyle: 'none', overflowY: 'auto', overflowX: 'hidden' }}>
          {!leftOpen ? (
            order.map((key) => {
      ...
      <main style={{
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 0 40px', position: 'relative' }}>
          ...
          {/* Drag handles + card row */}
          <div style={{
            display: 'flex', alignItems: 'flex-start',
            justifyContent: 'center', padding: '0 4px',
          }}>
            <DragHandle side="left"  onMouseDown={e => startDrag(e, 'left')}  t={t} active={isDragging} />

            {/* Card wrapper — controls animated width */}
            <div style={{
            ...
            <DragHandle side="right" onMouseDown={e => startDrag(e, 'right')} t={t} active={isDragging} />
          </div>
        </div>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <span style={{ color: t.textActive, fontSize: '13px', fontWeight: '600', whiteSpace: 'nowrap' }}>
            {selectedComponent ? COMPONENT_LABELS[selectedComponent] : 'Editor'}
          </span>
        </div>
        <div
        ...
        >
          {selectedComponent && BUILDERS[selectedComponent]
            ? BUILDERS[selectedComponent]
            : (
